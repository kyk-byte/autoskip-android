...
- `AutoSkipAccessibilityService`: lifecycle, package scope, delayed scan, click execution.
...
1. Only the two packages listed in `accessibility_service_config.xml` can deliver events.
...
- YouTube Music is off by default until device tests confirm current accessibility behavior.
